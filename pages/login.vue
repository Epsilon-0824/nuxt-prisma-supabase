...
const handleLogin = async () => {
    ...
    try {
        ...
        if(token){
            const authTokens: any = useCookie('auth_tokens')
...
const handleLoginWithPhone = async () => {
    ...
    try {
        ...
        message.value = 'OTP sent! Please check your phone for verification code.'
        status.value = true
    ...
    <div class="bg-base-200 h-[calc(100vh-50px)] flex items-center justify-center">
        <div class="card lg:card-side bg-base-100 shadow-xl max-w-4xl w-full">
            ...
            <div class="card-body lg:w-1/2">
                ...
                <p v-if="message" :class="status ? 'bg-success' : 'bg-error'" class="p-3 rounded text-white">{{ message
                    }}</p>
